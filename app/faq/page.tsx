...
type FaqItem = {
  question: string;
  answer: string;
  topic?: 'Strategy' | 'SEO' | 'Social' | 'Pricing' | 'Operations';
};
...
export const metadata: Metadata = pageMetadata({
  title: 'Digital Marketing FAQ | Walktopus',
  description:
    'Premium FAQ by Walktopus: expert answers on digital marketing strategy, SEO, social media management, pricing, and growth operations for businesses across India.',
  pathname: '/faq',
  keywords: [
    'digital marketing faq india',
    'walktopus pricing faq',
    'seo and social media questions',
    'local seo faq kolkata',
    'social media management faq india',
    ...
    <div className="relative mx-auto w-full max-w-7xl px-6 py-20 lg:py-28">
      ...
      <div className="mt-10 space-y-4">
        {FAQS.map((item, index) => (
          <details
            key={item.question}
            className="group border border-(--color-bg-secondary) bg-(--color-bg-light) p-6 transition-all duration-300 open:border-(--color-accent) open:bg-white open:shadow-[0_18px_48px_-32px_rgba(239,77,48,0.5)] md:p-7"
          >
            <summary className="flex cursor-pointer list-none items-start justify-between gap-6">
              <div>
                <p className="font-mono text-[11px] uppercase tracking-[0.13em] text-(--color-accent)">
                  Question {index + 1}
                  {item.topic ? ` - ${item.topic}` : ''}
                </p>
                <h2 className="mt-2 text-xl leading-tight text-(--color-text-dark) md:text-2xl">{item.question}</h2>
              </div>
              <span className="mt-1 inline-flex size-9 shrink-0 items-center justify-center border border-(--color-bg-secondary) bg-white text-(--color-text-dark) transition group-open:rotate-180 group-open:border-(--color-accent) group-open:text-(--color-accent)">
                <ChevronDown className="size-5" aria-hidden="true" />
              </span>
            </summary>

            <div className="mt-4 border-t border-(--color-bg-secondary) pt-4">
              <p className="max-w-5xl text-base leading-7 text-(--color-text)">{item.answer}</p>
            </div>
          </details>
        ))}
      </div>

      <div className="mt-14 border border-(--color-bg-secondary) bg-(--color-text-dark) p-8 text-(--color-bg) md:p-10">
        <p className="font-mono text-xs uppercase tracking-[0.12em] text-(--color-accent)">Need a direct recommendation?</p>
        <h2 className="mt-3 text-3xl leading-tight font-bold md:text-4xl">Discuss your current growth bottleneck and get a fit-based next step.</h2>
