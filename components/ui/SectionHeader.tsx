...
export function SectionHeader({ eyebrow, title, subtitle, centered = false, titleClassName }: SectionHeaderProps) {
  return (
    <div className={centered ? 'mx-auto max-w-3xl text-center' : 'max-w-3xl'}>
      {eyebrow ? (
        <p className="font-mono text-xs uppercase tracking-[0.15em] text-[var(--color-accent)]">{eyebrow}</p>
      ) : null}
      <h2 className={`mt-3 text-4xl font-extrabold leading-tight md:text-5xl${titleClassName ? ` ${titleClassName}` : ''}`}>{title}</h2>
